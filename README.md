...
Петров Петр:   
    хлеб 2  
   **47 TASK.**  
   ﻿Спроектируйте класс с именем Cat, с тремя полями внутри класса: String name, int age, int weight.
Необходимо написать класс Cat, больше ничего писать не надо!  
**48 TASK.**  
﻿Спроектируйте два класса: Cat, Dog;
В каждом из классов должно быть по два поля - name, age;  
